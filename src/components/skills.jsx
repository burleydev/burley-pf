    ...
    <div className='grid gap-x-[5rem] gap-y-[3rem] lg:grid-cols-4 xs:grid-cols-2' data-aos='fade-up'>
      {skills.map(skill => (
        <div key={skill.name} className='text-myBlack text-center'>
          ...
          <h4 className='my-3 lg:text-xl md:text-lg xs:text-md text-myGrey'>{skill.name}</h4>
        </div>
      ))}
      <div className='text-my text-center'>
        ...
        <h4 className='mt-4 lg:text-xl md:text-lg xs:text-md text-myGrey'>TypeScript</h4>
      </div>
    </div>
  </div>
);
